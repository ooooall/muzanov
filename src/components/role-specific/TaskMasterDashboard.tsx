...
export function TaskMasterDashboard({
  ...
  const stats = useMemo(() => getZoneStats(zones.map(z => z.status)), [zones])

  // ─── Assign operation ──────────────────────────────────────────────────────────
  const handleAssignOperation = useCallback(async (zoneId: string, operationId: string) => {
    const operation = operations.find(o => o.id === operationId)
    if (!operation) { toast.error('Операция не найдена'); return }
    ...
    const { error } = await supabase
      .from('zone_states')
      .update({ operation_type_id: operationId, updated_at: new Date().toISOString() })
      .eq('zone_id', zoneId)

    if (error) {
      // Revert to null so user sees the failure
      onPatchZone(zoneId, { operation_type_id: null, operation_types: null })
      toast.error('Не удалось назначить операцию')
      return
    }

    const log: TablesInsert<'activity_log'> = {
      zone_id: zoneId, user_id: userId, action: 'operation_assigned',
  ...
  }, [onPatchZone, operations, supabase, userId])
  ...
  const handleAssignWorker = useCallback(async (zoneId: string, workerId: string | null) => {
    ...
    if (error) {
      onPatchZone(zoneId, { assigned_worker_id: null, profiles: null })
      toast.error('Не удалось назначить исполнителя')
      return
    }

    const log: TablesInsert<'activity_log'> = {
      zone_id: zoneId, user_id: userId, action: 'worker_assigned',
  ...
  }, [onPatchZone, supabase, userId, workers])

  // ─── Save note ─────────────────────────────────────────────────────────────────
  const handleSaveNote = useCallback(async (zoneId: string, note: string) => {
    onPatchZone(zoneId, { notes: note, updated_at: new Date().toISOString() })
    ...
    try {
      ...
      if (!error) {
        const log: TablesInsert<'activity_log'> = {
          zone_id: zoneId, user_id: userId, action: 'note_added', details: { note },
        }
        supabase.from('activity_log').insert(log)
      }
      return error
  ...
  const handleResetBoard = useCallback(async () => {
    ...
    onResetAllZones() // instant local clear
    const response = await fetch('/api/admin/reset', { method: 'POST' })

    if (response.ok) {
      toast.success('Всё сброшено с нуля')
    } else {
      toast.error('Не удалось сбросить — проверьте подключение')
    }
  }, [onResetAllZones])
  ...
  return (
    <div className="mx-auto flex w-full max-w-screen-xl flex-col gap-5 overflow-x-hidden px-3 py-5 sm:px-4 sm:py-6">
      <div className="space-y-1">
        ...
        <p className="text-[13px] text-text-3">Постановка задач, контроль статусов и управление исполнителями.</p>
      </div>
...
function TaskLane({ zones, workers, onSelect }: {
  zones: ZoneWithState[]
  workers: Profile[]
  ...
  const sorted = useMemo(
    () => [...zones].sort((a, b) => {
      const order: ZoneStatus[] = ['review', 'in_progress', 'new', 'done']
